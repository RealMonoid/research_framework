...
| Data analysis report | Only for a concrete, scoped quantitative question requiring more than simple arithmetic; may profile data or evaluate a non-causal association/prediction under an explicit work order | **schemas/data_analysis_report.schema.json** plus **scripts/validate_data_analysis_report.py** |
...
A `data_analysis_report` is a bounded quantitative contribution to the
conductor. It records what data were actually used, their version and period,
variables and decision-time availability, data-quality checks, uncertainty,
stability, and limits. It may support descriptive or explicitly predictive
reasoning, but association is not causal evidence and one attractive backtest
is not validation. The report must not make a trading, risk, activation,
research-goal, or state decision. The conductor invokes it only for a concrete
information need, validates it, performs the full fingerprint comparison, and
keeps the interpretation. If the source data are not coherent or sufficient,
the report records `NOT_TESTABLE` or `BLOCKED`; it never transfers repetitive
manual acquisition work to the owner. The validator is
`scripts/validate_data_analysis_report.py`.
