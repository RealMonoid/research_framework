...
**Audited revision:** `codex/test-validity-pipeline` (pre-merge)
...
does not create that state or call the specialist. Since ADR-018, it opens and
validates completed outcome contracts and pipeline assessments, including their
execution evidence. Other artifact classifications remain caller supplied. A
caller can still skip the router or fabricate a mutually consistent set of
local files; the router cannot independently authenticate the producer.
...
| **HG-09** | Validation cannot be frozen until outcome roles, falsifiers, multiplicity families, mechanical coupling, boundaries and decision consequences were fixed before results. | The router opens completed contracts; v2 validation checks the canonical protocol, unique fingerprint commitments, original freeze and observed execution. Result routes require compatible assessed evidence. | Missing/conflicting protocols fail; boundary or inspection deviations are `INVALID_TEST` and cannot support prediction/edge. | `scripts/test_outcome_evidence_contract.py`, `scripts/test_execution_controls.py`, schema and orchestration cases. | Host invocation, full observation, data fitness and producer authenticity remain caller-enforced. See ADR-018. | **B+C+D** |
| **HG-10** | The unchanged complete pipeline cannot enter validation until repeated relevant negative controls and a known-effect sentinel pass. | The router opens assessed controls and verifies locked plans, pipeline/code hashes, isolated execution receipts, seeds, numerical results and reference-generator truth. Repeated random walks alone fail. | Only verified assessed `PASS` is complete; unsupported generators, absent execution, altered budgets or contradictory results cannot pass. | `scripts/test_pipeline_integrity_assessment.py`, `scripts/test_execution_controls.py`, schema and orchestration cases. | Current truth verifier covers the paired-uniform synthetic reference only. Market-structure adequacy, dependency completeness, sandboxing and authenticated producer evidence are not established. | **B+C+D** |
...
| **HG-29** | Every empirical hypothesis and test requires evidence that the exact dataset can answer the requested question; unmet or unresolved material quality, coverage, resolution, timing, or execution-data requirements stop the affected test without disproving the hypothesis. Research must not proceed when the required data cannot be obtained as a coherent dataset, and data-quality assurance must not be replaced by arbitrary screenshot quotas or repetitive manual chart extraction imposed on the owner. | Root agent instructions and the conductor require automation-first checks, one reusable snapshot where possible, and a named residual risk before any bounded manual review. The prospective data-fitness gate specifies the eventual structured decisions. | A compliant agent reports the data path as limited, remediable, not testable, or blocked instead of silently weakening the strategy or transferring repetitive acquisition work to the user. | No executable regression yet proves that a live agent refuses an arbitrary manual-review request or repeated TradingView loading. | The prerequisite is binding now; only the dedicated structured data-fitness artifact and validator are still planned. Current enforcement depends on the host reading and following the instructions and honestly assessing the burden. | **D+P+M** |
| **HG-30** | An observable workflow-control bypass must be diagnosed and bounded without changing the effective research state or inventing a new research gate for ordinary work. | The conditional [`framework-control-reviewer`](agents/framework-control-reviewer.md) receives an explicit request or concrete signal and returns a structured report validated by [`validate_framework_control_review.py`](scripts/validate_framework_control_review.py). The AI-Psychiatry plugin may supply the same provider-neutral modes, but `AGENTS.md` remains controlling. | The report records a confirmed finding, a narrow corrective action or blocker, and a regression check. A proposed material research change remains visible and goes through the full fingerprint and user-decision path. | `scripts/test_framework_control_review.py` and the synthetic control-review fixture. | This is caller-enforced and conditional; a clean report does not prove live-agent reliability, and no plugin invocation is claimed when the plugin is unavailable. The reviewer cannot replace the scientific-philosophy or causal-identification route and must not inspect private chain-of-thought. | **B+C+D+P** |
| **HG-31** | Every research task must remain tied to the user's scope, use at most one bounded delegation level, require evidence for material conclusions, avoid equivalent repeat checks without new information, and reject completion claims without validated evidence. | The conductor instructions apply these controls on every task, and routing decision schema version 1.9.0 records them as constant machine-checked fields (`control`). The control-reviewer and AI-Psychiatry modes can diagnose an observed bypass but are not required for the baseline controls. | The router or schema rejects a relaxed control contract; the conductor keeps the task incomplete, blocks the transition, or records a visible proposal when the required evidence is missing. | Negative schema cases in `scripts/test_schemas.py`, route-output tests in `scripts/test_research_orchestration.py`, and the conductor/quickstart documentation. | The current repository still relies on the caller to invoke the router and to report truthful observations; a hand-written route artifact can be fabricated. Live-agent trajectory evaluation is required before claiming end-to-end enforcement. | **B+C+D+P** |
| **HG-32** | A quantitative data contribution must not silently become a trading, causal, validation, or research-state decision. Provenance, data quality, decision-time availability, uncertainty, and trading-specific boundaries must be visible before the conductor accepts it. | The conditional `DATA_ANALYSIS` route selects the bounded `data-analyst`; `schemas/data_analysis_report.schema.json` fixes the no-action and no-causal boundary, and `scripts/validate_data_analysis_report.py` checks provenance, quality, leakage, availability, uncertainty, and disposition. The conductor then performs the complete fingerprint comparison. | An invalid, unsupported, unavailable, or causally overreaching report is rejected or remains `NOT_TESTABLE`/`BLOCKED`; the effective research state and capital decision remain unchanged. | `scripts/test_data_analysis_report.py`, schema contract tests, and the Data route cases in `scripts/test_research_orchestration.py`. | Invocation and truthful input-data description remain caller-enforced; this report is not the planned prospective data-fitness gate and does not prove that a live provider behaved correctly. | **B+C+D+P** |
| **HG-33** | The owner must be able to see the current position, next framework action, subsequent step, own required action, and a decision-ready recovery path before a material research transition or blocker is accepted. | Every routing decision now requires a `user_interaction` progress brief. Required choices carry at least two weighted options and a recommendation. A `BLOCKED` decision requires a separate problem-record reference and recovery options; the `problem_record` schema and semantic validator require model identity, timestamps, description, impact, and one recommended recovery option. | A route without the progress brief or a blocked route without a problem reference and recovery options fails schema or router validation. The conductor must not advance around a documented problem. | `scripts/test_research_orchestration.py`, `scripts/test_problem_record.py`, and schema-contract negative cases. | Creation, storage, and validation of the separate private file remain conductor-enforced; the router cannot discover a problem that an agent never records or prove that a stable reference resolves to the intended private file. Live-agent evaluation must test these omissions. | **B+C+D+P** |
| **HG-34** | A mandatory specialist must not be declared unavailable until the live runtime's invocation capabilities have been inspected, recorded, and matched to the routed work order. | Schema version 1.9.0 checkpoints store `specialist_capability`; each specialist route requires a separate `specialist_capability_check` bound to the routing decision. Its semantic validator forces `AVAILABLE` when any inspected interface satisfies all requirements, treats incomplete discovery as `UNKNOWN`, and permits `UNAVAILABLE` only after a complete search. | A suitable internal interface makes invocation the required next action. The router rejects a specialist-unavailability blocker when the checkpoint says `AVAILABLE` or `UNKNOWN`, or when the blocker does not reference the same capability check. | `scripts/test_specialist_capability_check.py`, specialist cases in `scripts/test_research_orchestration.py`, and schema negative cases. | Discovery remains caller-enforced: the repository cannot independently enumerate a host's live tools when an agent omits them from the record. Live-agent evaluation must test truthful inventory capture. | **B+C+D+P** |
