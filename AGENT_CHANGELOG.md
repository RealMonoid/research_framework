...
- **Why**: **Problem description**: The completed cross-disciplinary synthesis would otherwise remain disconnected from the authoritative feature-planning surface and could be overlooked or reimplemented as an unbounded post-hoc root-cause process. **Decision context**: The user explicitly requested that the document be referenced from the feature list and uploaded; the request did not authorize changing roadmap priority or implementing the proposed synthesis. **Protected invariants**: `PLANNED_FEATURES.md` remains the sole shared roadmap; the existing priority order is unchanged; research evidence is not presented as implementation or validation; future work must protect a concrete research or capital decision and preserve the current specialist boundaries.
- **Verification**: Re-read the affected roadmap section immediately before editing; confirmed the link target exists; `git diff --check` passed. No framework rule, priority, strategy, market data, backtest, test, or trading state was changed.
