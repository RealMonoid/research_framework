...
- Maintain `AGENT_CHANGELOG.md`: check it before starting work to review recent changes by other agents, and record every completed change in English with exact timestamp (date and time), full agent model name and version number (e.g. `Gemini 3.8 Flash`, `ChatGPT 5.6 Sol`), affected files, detailed WHAT was changed, detailed WHY it was changed, and verification status.
