...
NEGATIVES: list[tuple[str, str, str, Mutation]] = [
    ...
    ("specialist handoff must compare all six identity dimensions", "examples/routing_decision.pre_operationalization.json", "schemas/routing_decision.schema.json", set_value(("identity_guard", "compared_dimensions"), ["research_question", "strategy", "market", "time_horizon", "trigger"])),
    ("identity report must contain each dimension exactly once", "examples/research_identity_check.unchanged.json", "schemas/research_identity_check.schema.json", set_value(("comparisons", 5, "dimension"), "trigger")),
    ("unchanged identity report cannot list a changed target", "examples/research_identity_check.unchanged.json", "schemas/research_identity_check.schema.json", set_value(("changed_dimensions",), ["target"])),
    ("detected drift requires a report reference", "examples/orchestration_state.prose_strategy.json", "schemas/orchestration_state.schema.json", set_value(("handoff_control", "status"), "DRIFT_DETECTED")),
