...
NEGATIVES: list[tuple[str, str, str, Mutation]] = [
    ...
    ("data specialist cannot make a trading action", "examples/data_analysis_report.synthetic.json", "schemas/data_analysis_report.schema.json", set_value(("no_trading_action",), False)),
    ("data specialist cannot make a causal claim", "examples/data_analysis_report.synthetic.json", "schemas/data_analysis_report.schema.json", set_value(("no_causal_claim",), False)),
    ("predictive claim cannot carry a causal identification pass", "examples/causal_identification_assessment.hfi_pass.json", "schemas/causal_identification_assessment.schema.json", set_value(("requested_claim_level",), "ASSOCIATIONAL_PREDICTIVE")),
    ("causal discovery alone cannot receive identification pass", "examples/causal_identification_assessment.hfi_pass.json", "schemas/causal_identification_assessment.schema.json", set_value(("design", "design_family"), "CAUSAL_DISCOVERY_ONLY")),
    ("causal pass requires a falsification or negative control", "examples/causal_identification_assessment.hfi_pass.json", "schemas/causal_identification_assessment.schema.json", set_value(("diagnostics", "falsification_or_negative_controls"), [])),
    ...
    ("capability check requires active tool inventory inspection", "examples/specialist_capability_check.internal_agent.json", "schemas/specialist_capability_check.schema.json", set_value(("discovery", "active_tool_inventory_checked"), False)),
    ...
    ("routing decision requires specialist capability discovery", "examples/routing_decision.pre_operationalization.json", "schemas/routing_decision.schema.json", set_value(("control", "specialist_capability_check_required_before_invocation"), False)),
    ("routing decision requires complete discovery before unavailable", "examples/routing_decision.pre_operationalization.json", "schemas/routing_decision.schema.json", set_value(("control", "specialist_unavailability_requires_complete_discovery"), False)),
    ...
    ("routing decision bounds delegation depth", "examples/routing_decision.pre_operationalization.json", "schemas/routing_decision.schema.json", set_value(("control", "delegation_max_depth"), 2)),
    ("routing decision binds conclusions to evidence", "examples/routing_decision.pre_operationalization.json", "schemas/routing_decision.schema.json", set_value(("control", "conclusions_require_evidence"), False)),
    ("routing decision requires new evidence for repeats", "examples/routing_decision.pre_operationalization.json", "schemas/routing_decision.schema.json", set_value(("control", "repeat_requires_changed_evidence"), False)),
    ("routing decision binds completion to validation", "examples/routing_decision.pre_operationalization.json", "schemas/routing_decision.schema.json", set_value(("control", "completion_requires_validated_evidence"), False)),
    ("philosophy route requires philosophy specialist", "examples/routing_decision.pre_operationalization.json", "schemas/routing_decision.schema.json", set_value(("selected_agent",), "condition-inquiry-analyst")),
    ("routing work order permits only one attempt", "examples/routing_decision.pre_operationalization.json", "schemas/routing_decision.schema.json", set_value(("work_order", "max_attempts"), 2)),
    ("protected route must compare the full material research state", "examples/routing_decision.pre_operationalization.json", "schemas/routing_decision.schema.json", set_value(("fingerprint_guard", "comparison_scope"), "NONE")),
    ("fingerprint cannot claim unprotected effective artifacts", "examples/research_fingerprint.prose_strategy.json", "schemas/research_fingerprint.schema.json", set_value(("completeness", "all_effective_material_artifacts_protected"), False)),
    ("unchanged fingerprint check cannot block candidate", "examples/research_fingerprint_check.unchanged.json", "schemas/research_fingerprint_check.schema.json", set_value(("candidate_may_become_effective",), False)),
    ("change proposal requires its check and proposal references", "examples/orchestration_state.prose_strategy.json", "schemas/orchestration_state.schema.json", set_value(("change_control", "status"), "CHANGE_PROPOSED")),
